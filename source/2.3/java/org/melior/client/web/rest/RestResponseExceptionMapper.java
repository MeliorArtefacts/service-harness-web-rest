...
/**
 * Generates a standard {@code RemotingException} from an exception payload which
 * is received when an exception is raised in an application which processes HTTP
 * REST requests.
 * <p>
 * The mapper fully supports the standard exception payload that is defined by Melior,
 * as well as the default exception payload that is defined by Spring Boot, but the
 * mapper will function equally well with any exception payload that contains at
 * the very least a "message" parameter.
 * <p>
 * The mapper converts the HTTP status code of the HTTP response which contains the
 * exception payload into an appropriate exception type for the {@code RemotingException}.
 * @author Melior
 * @since 2.0
 */
